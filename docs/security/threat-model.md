...
**Control status:** Partially implemented — `TL-0105` supplies and verifies the closed typed provider/normalization boundary, provenance, value/count/source bounds, cancellation/timeout outcomes, and fail-closed unavailable/error mapping. Concrete Windows providers, provider-specific parsers/bounds, policy evaluation, persistence, and fresh re-observation remain planned in their owning tasks.  
